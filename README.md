...
### 成绩概览（首页）
![成绩概览](screenshots/07-overview.png)
...
- **成绩概览**：首页一览每个孩子最近几次考试的折算总分、排名、升降与迷你趋势，多孩子一屏对比
...
- **V0.3** 新增「成绩概览」首页（按学生卡片 + 迷你趋势）；安全加固（修复参数配置存储型 XSS、CSV 公式注入防护）、数据修复（编辑态串数据、重名学生拦截、趋势统计口径、全角数字导入、0 分可保存等）、界面优化（分辨率适配、录入表格列宽、排名项独立、按钮对齐、存储提醒可按版本关闭）、导入规则调整（原始成绩 0–150、折算比例 0–1.0）、老浏览器兼容
